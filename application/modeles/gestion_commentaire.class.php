...
class GestionCommentaire
{
    ...
    public static function getLesCommentaires()
    {
        return self::SelectAll("commentaire");
    }

    public static function getLesCommentaireByArticle($art) {
        self::seConnecter();

        self::$requete = "select * from articles a, utilisateur u, commentaire c where a.idArticle = c.idArticles and u.idUti = c.idUser and c.idArticles = :idArt";

        self::$pdoStResults = self::$pdoCnxBase->prepare(self::$requete);
        self::$pdoStResults->bindValue('idArt', $art);
        self::$pdoStResults->execute();
        self::$resultat = self::$pdoStResults->fetchAll();

        self::$pdoStResults->closeCursor();
        return self::$resultat;
        VariablesGlobales::$leRestult = self::$resultat;
