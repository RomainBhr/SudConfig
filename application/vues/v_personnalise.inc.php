<div class="corp-article">
    ...
    <article>
        <h1><br>Custom ton clavier <u>#<?= $cat->compteur ?></u>
            <?php if ($cat->favori == 1){ ?>
                <span style="color: #00a1ff">★</span>
            <?php } ?>
        </h1>
        <h4 style="color: black; text-align: left;"><span class="red">*</span> Le prix d'une touche est de 0,10€</h4>
        <div class="larticle">
            <form method="post">
                    ...
                    <tr>
                        <?php foreach (VariablesGlobales::$touche as $unetouche){ ?>
                            <?php if ($unetouche->libelleTou == "echap") { ?>
                                <td class="touche" style="background: <?= $unetouche->couleurperso ?>; color: <?= $unetouche->txtcouleur ?>"><a href="#oModal<?= $unetouche->libelleTou ?>" class="white-a"><?= $unetouche->libelleTou ?></a></td>
                            <?php }
                        }
                        ...
                        foreach (VariablesGlobales::$touche as $unetouche){
                            ...
                            <?php } if ($unetouche->libelleTou == "entrée"){?>
                                <td class="touche" rowspan="2" style="background: <?= $unetouche->couleurperso ?>; color: <?= $unetouche->txtcouleur ?>"><a href="#oModal<?= $unetouche->libelleTou ?>" class="white-a"><?= $unetouche->libelleTou ?></a></td>
                            <?php }
                        }
                        echo "</tr><tr>";
                        foreach (VariablesGlobales::$touche as $unetouche){
                        ...
                        foreach (VariablesGlobales::$touche as $unetouche){
                            if ($unetouche->libelleTou == "▼"){ ?>
                                <td class="touche" colspan="2" style="background: <?= $unetouche->couleurperso ?>; color: <?= $unetouche->txtcouleur ?>"><a href="#oModal<?= $unetouche->libelleTou ?>" class="white-a"><?= $unetouche->libelleTou ?></a></td>
                            <?php }
                        }
                        echo "<td></td>";
                        foreach (VariablesGlobales::$touche as $unetouche){
                        ...
                        foreach (VariablesGlobales::$touche as $unetouche){ ?>
                            ...
                            <?php }if ($unetouche->libelleTou == "space"){ ?>
                                <td class="touche" colspan="4" style="background: <?= $unetouche->couleurperso ?>; color: <?= $unetouche->txtcouleur ?>"><a href="#oModal<?= $unetouche->libelleTou ?>" class="white-a"><?= $unetouche->libelleTou ?></a></td>
                        ...
                        <div>
                            ...
                            <h2>
                                ...
                                <p> Vous avez choisit la touche <u><?= $unetouche->libelleTou ?></u>
                                    <br>qui a pour couleur le <span style="background: <?= $unetouche->couleurperso ?>; border: 1px solid #000; padding: 10px; width: 10px; height: 10px; font-size: 0px;margin-left: 10px;"> </span></p>
                                <form method="post" class="formulaire" style="text-align: center; display: initial;">
                                    ...
                                    <select name="couleur" class="bouton">
                                        ...
                                        <option data-target="cl1" value="<?= $uneCouleur->couleurHtag ?>" style="background: <?= $uneCouleur->couleurHtag ?>" class="couleur"><?= $uneCouleur->libelleCouleur ?></option>
                                        <?php } ?>
                                        <input name="default" type="hidden" value="<?= $unetouche->couleurperso ?>">
                                    </select>
                                    <input data-role="update" id="update" value="Valider cette couleur" name="subcouleur" type="submit" class="submit bouton" />
                                </form>
                                <?php if ($unetouche->couleurperso != "#000"){ ?>
                                    <form method="post" style="text-align: center; display: initial;">
                                        <input name="revenir" type="submit" value="Revenir à la couleur d'origine" class="submit bouton" >
